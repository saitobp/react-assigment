...
function resolvePlayer(authorization: string | undefined): Player | undefined {
  if (noAuth) return players[0]
  if (!authorization) return undefined
  return players.find(p => p.accessToken === authorization.replace('Bearer ', ''))
}
...
app.post('/login', (req, res) => {
  ...
  if (!player) return res.status(400).json({ message: 'Email ou senha inválidos' })
...
app.post('/bet', (req, res) => {
  ...
  const player = resolvePlayer(authorization)
  if (!player) return res.status(401).json({ message: 'Token inválido' })

  if (player.balance < amount) return res.status(400).json({ message: 'Saldo insuficiente' })

  if (amount < 1) return res.status(400).json({ message: 'Valor mínimo para aposta é 1' })
...
app.get('/my-bets', (req, res) => {
  ...
  if (!page || !limit) return res.status(400).json({ message: 'Parâmetros inválidos' })
  ...
  const data = bets.slice((pageNum - 1) * limitNum, pageNum * limitNum)
...
app.delete('/my-bet/:id', (req, res) => {
  ...
  if (bet.status === 'canceled') return res.status(400).json({ message: 'Aposta já cancelada' })
...
app.get('/my-transactions', (req, res) => {
  ...
  const transactions = player.transactions
    .filter(
      transaction =>
        (!id || transaction.id === id) && (!type || transaction.type === type) && transaction,
    )
    .sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())

  const total = transactions.length
  const data = transactions.slice((pageNum - 1) * limitNum, pageNum * limitNum)
...
app.listen(port, () => {
  ...
  if (noAuth) console.log('Running in --no-auth mode: token validation is disabled')
})
